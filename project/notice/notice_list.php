...
<div style="width:90%;margin:0 auto;text-align:center;">
  ...
  <table class="table">
    <thead class="thead-dark">
      ...
      <th scope="col" style="width:5%;text-align:center;">No</th>
        <th scope="col" style="width:55%;text-align:center;">Title</th>
        <th scope="col" style="width:15%;text-align:center;">Name</th>
        <th scope="col" style="width:15%;text-align:center;">Date</th>
        <th scope="col" style="width:10%;text-align:center;">Hits</th>
      </tr>
    </thead>
    <tbody>
    <?php
    ...
    $sql2 = "select * from notice order by notice_top desc,notice_idx desc limit $start_num, $list";
    ...
    ?> 
      <tr>
        ...
        ?>
        <a href="notice_view.php?notice_idx=<?php echo $rows['notice_idx'];?>">
        <?php 
        $number--;
        ?>
        ...
        if(strlen($title) > 25){ //strlen : 자릿수
          $title = str_replace($rows['notice_title'],mb_substr($rows['notice_title'],0,25,"utf-8")."...",$rows['notice_title']);
        }
        
        //$wdate = $rows['notice_wdate'];
        //$timenow = date("Y-m-d");

        ?>

        <td style="width:55%;text-align:left;">
        <a href="notice_view.php?notice_idx=<?php echo $rows['notice_idx'];?>&page=<?php echo $page?>">
        <?php echo $title;?></a>
        <?php
        if(time() - strtotime($rows['notice_wdate']) <= 60 * 60 * 24 * 2){
        ...
        <td style="width:15%;text-align:center;"><?php echo $rows['notice_name']?></td>
        
        <td style="width:15%;text-align:center;"><?php echo substr($rows['notice_wdate'],0,10);?></td>
        <td style="width:10%;text-align:center;"><?php echo $rows['notice_hit'];?></td>
      </tr>
    <?php
    }
    }else{
  ...
  <div style="text-align:center;">
    <nav aria-label="Page navigation example">
      <ul class="pagination justify-content-center">
        ...
        <li class="page-item">
        <?php
        if($block_num >= $total_block){ //현재블록이 블록의 총 개수보다 크거나 같다면
        } else {
          $nex = $page + 1;
  ...
  <a class="btn btn-dark" href="notice_write.php" role="button">글쓰기</a>
  </div>
<?php
}
?>
